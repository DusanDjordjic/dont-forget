...
unsigned long int       io_trim_str_left(char** str); // Trims a string from left and returns number of removed characters. -1 if error happend
unsigned long int       io_trim_str_right(char** str); // Trims a from right string and returns number of removed characters. -1 if error happend
#endif
